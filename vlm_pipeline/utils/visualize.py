...
def draw_boxes(
    ...
    for item in boxes:
        ...
        label = clean_html_tags(str(item.get(label_key, "")))[:60]
        if not label:
            continue
...
def save_page_comparison(
    ...
    if "detect" in page_tasks and page_tasks["detect"].get("text_lines"):
        overlay = draw_boxes(image, page_tasks["detect"]["text_lines"], label_key="text")
    elif "layout" in page_tasks and page_tasks["layout"].get("blocks"):
        overlay = draw_boxes(image, page_tasks["layout"]["blocks"], label_key="label", color="blue")

    if overlay is None:
        return None

    path = output_dir / f"page_{page_index}_comparison.png"
    side_by_side(image, overlay).save(path)
    return str(path)
...
def save_overlays(
    ...
    if "text_lines" in page_result:
        ...
        draw_boxes(image, page_result["text_lines"], label_key="text").save(path)
        saved.append(str(path))

    if "blocks" in page_result:
        path = output_dir / f"page_{page_index}_layout_bbox.png"
        draw_boxes(image, page_result["blocks"], label_key="label", color="blue").save(path)
        saved.append(str(path))
