...
from vlm_pipeline.backends import BACKENDS
from vlm_pipeline.image_utils import resize_long_edge
from vlm_pipeline.pdf_io import load_pages
from vlm_pipeline.visualize import save_overlays, save_page_comparison


class DocumentVLMPipeline:
    """Surya-style open VLM pipeline: one model, multiple prompt tasks."""
    ...
    def __init__(
        ...
        self.quant = quant
        self.fast = fast
        self.dpi = dpi or (self.FAST_PRESETS["dpi"] if fast else None)
        self.max_image_size = max_image_size or (
            self.FAST_PRESETS["max_image_size"] if fast else 1024
        )
        self.max_tokens = max_tokens or (
            self.FAST_PRESETS["max_tokens"] if fast else 2048
        )
        self.default_tasks = self.FAST_PRESETS["tasks"] if fast else ["ocr"]
    ...
    def run(
        ...
        for page_idx in page_range or [0]:
            ...
            for task in tasks:
                dpi = self.dpi or self.TASK_DPI[task]
                ...
                if save_images and task in {"detect", "layout"}:
                    overlays.extend(
                        save_overlays(
                            image,
                            task_result,
